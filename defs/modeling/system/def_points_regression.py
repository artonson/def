...
class DEFPointsRegression(LightningModule):
    ...
    def _shared_eval_step(self, batch, batch_idx: int, dataloader_idx: Optional[int], partition: str):
        ...
        self.mrecall_pos[partition][dataloader_idx].update(result['distances'] < 4 * resolution, batch['distances'] < 4 * resolution)
        self.mrecall_neg[partition][dataloader_idx].update(result['distances'] < 4 * resolution, batch['distances'] < 4 * resolution)
