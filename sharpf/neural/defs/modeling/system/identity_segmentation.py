...
class IdentitySegmentation(LightningModule):
    ...
    def forward(self, x):
        ...
        out: Dict[str, torch.Tensor] = {'preds_sharp': result}
        return out
    ...
    def _shared_eval_step(self, batch, batch_idx: int, dataloader_idx: Optional[int], partition: str):
        ...
        result['preds_sharp'] = result['preds_sharp'].view(result['preds_sharp'].size(0), -1)
        gt = batch['distances']
        gt = gt.view(gt.size(0), -1)

        self.mrecall_1r[partition][dataloader_idx].update(result['preds_sharp'], gt < 1 * resolution)
        self.mfpr_1r[partition][dataloader_idx].update(result['preds_sharp'], gt < 1 * resolution)
        self.mrecall_4r[partition][dataloader_idx].update(result['preds_sharp'], gt < 4 * resolution)
        self.mfpr_4r[partition][dataloader_idx].update(result['preds_sharp'], gt < 4 * resolution)
