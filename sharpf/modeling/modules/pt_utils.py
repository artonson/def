...
class MaskedMaxPoolStride(nn.Module):
    def __init__(self, stride, radius, nsample, sampleDl):
        super(MaskedMaxPoolStride, self).__init__()
        self.stride = stride
        self.radius = radius
        self.nsample = nsample
        self.sampleDl = sampleDl
    ...
    def forward(self, xyz, mask, features):
        npoint = features.size(2) // self.stride

        # sub sample
        sub_xyz, sub_mask = masked_grid_subsampling(xyz, mask, npoint, self.sampleDl)
        sub_xyz = sub_xyz.contiguous()
        sub_mask = sub_mask.contiguous()
...
class MaskedAvgPoolStride(nn.Module):
    def __init__(self, stride, radius, nsample, sampleDl):
        super(MaskedAvgPoolStride, self).__init__()
        self.stride = stride
        self.radius = radius
        self.nsample = nsample
        self.sampleDl = sampleDl
