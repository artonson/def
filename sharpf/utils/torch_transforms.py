...
class AbstractSharpnessTransform(ABC, Callable):
    @abstractmethod
...
class RandomScale(AbstractSharpnessTransform):
...
class Center(AbstractSharpnessTransform):
    def __call__(self, data, target):
        # center: E p_i = 0
...
class NormalizeL2(AbstractSharpnessTransform):
    def __call__(self, data, target):
        # scale: sup_i ||p_i|| = 1
...
class CompositeSharpnessTransform(AbstractSharpnessTransform):
...
class TypeCast(AbstractSharpnessTransform):
