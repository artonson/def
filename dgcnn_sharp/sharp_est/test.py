...
parser.add_argument('--model', default='dgcnn_sharp', help='Model name [default: model]')
...
TEST_DATASET = modelnet_h5_dataset.ModelNetH5Dataset(os.path.join(BASE_DIR, 'data/4pps_sel_short/test_files.txt'), batch_size=BATCH_SIZE, npoints=NUM_POINT, shuffle=False)
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    return np.array(pcs).reshape(-1, NUM_POINT, 3), np.array(losses).reshape(-1, NUM_POINT), np.array(preds).reshape(-1, NUM_POINT, 1), np.array(labels).reshape(-1, NUM_POINT, 1)
...
if __name__=='__main__':
    ...
    '''file = open('../../sharp/dataset_fine/abc_05_06_sharp_1024_patches_normalized_4pps_selected_short_curves_surfaces_names_test_new.txt', 'r')
    names = file.readlines()
    file.close()'''
    for i in range(len(pcs)):
        out = np.hstack((pcs[i], preds[i].reshape(len(labels[i]),1), labels[i].reshape(len(labels[i]),1), losses[i].reshape(len(losses[i]),1)))
        np.savetxt("../../sharp/networks/predictions/dgcnn/log_4pps_sel_short_80k_51ep_aug/%s.csv" % names[i][8:-1], out, delimiter=",", header='x,y,z,s,s_t,loss')
