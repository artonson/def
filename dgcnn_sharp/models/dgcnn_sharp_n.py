...
def get_model(point_cloud, is_training, bn_decay=None):
  ...
  # norm = tf.norm(net, axis = 2)
  # norm = tf.stack([norm,norm,norm], axis = 2)
  # net = net / norm
  return net
...
def get_loss(pred, label):
    """ pred: BxNxC,
        label: BxN, """
    classify_loss = tf.losses.sigmoid_cross_entropy(multi_class_labels=label, logits=pred, reduction = tf.losses.Reduction.MEAN)
