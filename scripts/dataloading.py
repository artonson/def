...
from vectran.data.chunked import ChunkedConcatDatasetLoader, ChunkedDatasetLoader
from vectran.data.prefetcher import CudaPrefetcher
from vectran.data.preprocessed import PreprocessedDataset


def make_combined_loaders(data_root, train_batch_size, val_batch_size, memory_constraint, shuffle_train=True, prefetch=False, device=None, handcrafted_val_part=.1, mini_val_batches_n_per_subset=None):
    # prepare datasets
    ...
    # prepare loaders
    train_loader = ChunkedConcatDatasetLoader(datasets_train, batch_size=train_batch_size, memory_constraint=memory_constraint, shuffle=shuffle_train)
    ...
    if mini_val_batches_n_per_subset is not None:
        ...
        dataset_val_mini = ConcatDataset([dataset.slice(0, mini_valset_size) for dataset in [sesyd_val] + handcrafted_val])
...
def make_sesyd_loaders(data_root, train_batch_size, val_batch_size, memory_constraint, shuffle_train=True, prefetch=False, device=None, mini_val_batches_n_per_subset=None):
    # prepare datasets
    sesyd_train = PreprocessedDataset(os.path.join(data_root, 'preprocessed/sesyd_walls.train'))
    sesyd_val = PreprocessedDataset(os.path.join(data_root, 'preprocessed/sesyd_walls.val'))

    # prepare loaders
    train_loader = ChunkedDatasetLoader(sesyd_train, batch_size=train_batch_size, memory_constraint=memory_constraint, shuffle=shuffle_train)
...
def make_handcrafted_loaders(data_root, train_batch_size, val_batch_size, memory_constraint,
    ...
    if None is not handcrafted_train_paths and None is not handcrafted_val_paths:
        ...
        if None is handcrafted_train_paths:
            ...
            _handcrafted_datasets = [PreprocessedDataset(os.path.join(data_root, 'preprocessed/synthetic_handcrafted', path))
                                     for path in handcrafted_train_paths]
