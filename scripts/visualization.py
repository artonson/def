...
from vectran.renderers.cairo import render as _render, render_with_skeleton as _render_with_skeleton
...
def make_images(lines_out, lines_gt, patch_size, patch_padding=(2, 2), patch_padding_color=230,
    ...
    if with_skeleton:
        ...
        renderer = lambda lines: render_without_skeleton(lines, patch_size=patch_size)[..., None]
    ...
    # render overlay
    if with_skeleton:
        renderer = lambda lines_out, lines_gt: render_overlay_colored(lines_out, lines_gt, patch_size)
    else:
        renderer = lambda lines_out, lines_gt: render_overlay(lines_out, lines_gt, patch_size)[..., None]
...
def render_with_skeleton(lines, patch_size, skeleton_line_width=2, skeleton_node_size=8):
    ...
    return _render_with_skeleton({graphics_primitives.PrimitiveType.PT_LINE: scaled_primitives}, patch_size,
                                 data_representation='vahe', line_width=skeleton_line_width,
                                 node_size=skeleton_node_size)


def render_without_skeleton(lines, patch_size):
    scaled_primitives = postprocess_primitives(lines, patch_size[0])
    return _render({graphics_primitives.PrimitiveType.PT_LINE: scaled_primitives}, patch_size,
                   data_representation='vahe')


def render_overlay(lines_out, lines_gt, patch_size):
    scaled_out = postprocess_primitives(lines_out, patch_size[0])
    scaled_gt = postprocess_primitives(lines_gt, patch_size[0])
    out_image = _render({graphics_primitives.PrimitiveType.PT_LINE: scaled_out}, patch_size, data_representation='vahe')
    gt_image = _render({graphics_primitives.PrimitiveType.PT_LINE: scaled_gt}, patch_size, data_representation='vahe')

    return np.uint8(255) - ((np.uint8(255) - out_image) // np.uint8(2) + (np.uint8(255) - gt_image) // np.uint8(2))


def render_overlay_colored(lines_out, lines_gt, patch_size):
    scaled_out = postprocess_primitives(lines_out, patch_size[0])
    scaled_gt = postprocess_primitives(lines_gt, patch_size[0])
    out_image = _render({graphics_primitives.PrimitiveType.PT_LINE: scaled_out}, patch_size, data_representation='vahe')
    gt_image = _render({graphics_primitives.PrimitiveType.PT_LINE: scaled_gt}, patch_size, data_representation='vahe')
