...
__usage="
Usage: $0 -a abc_input_dir -i input_file -o output_file [-v] -s schema
...
VERBOSE=false
while getopts "a:c:i:o:vf:s:" opt
do
    case ${opt} in
        c) CHUNK=$OPTARG;;
...
if [[ ! ${SCHEMA} ]]; then
    echo "schema is not set" && usage && exit 1
fi
...
SCRIPT="${CODE_PATH_CONTAINER}/scripts/data_scripts/compute_mesh_patches.py"
