...
from vectran.simplification.utils import pointsC_are_indistinguishable, unique_points
from vectran.simplification.simplify import bezier as simplify_bezier
...
class Bezier(Primitive):
    ...
    def simplified(self, distinguishability_threshold):
        r'''Returns
            ...
            3. (self,) otherwise
        '''
        if all(pointsC_are_indistinguishable(self.segment[0], p, distinguishability_threshold) for p in self.segment[1:]):
            return None,
        
        if distinguishability_threshold == 0:
            return self.copy(),

        segments = simplify_bezier(self.segment, distinguishability_threshold, max_segments_n=Bezier.MAX_LINEAR_SEGMENTS)
...
Primitive._seg2prim = {svgpathtools.Line: Line, svgpathtools.CubicBezier: Bezier, svgpathtools.QuadraticBezier: Bezier.from_quadratic}
