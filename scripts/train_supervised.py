...
from vectran.util.logging import create_logger
import vectran.util.dataloading as dataloading
...
def main(options):
    ...
    def validate(loader, log_i, prefix=''):
        ...
        for batch_j, batch_data_val in enumerate(loader):
            ...
            y_true_vector = vmetrics.batch_numpy_to_vector(y_true.cpu().numpy(), RASTER_RES)
            y_pred_vector = vmetrics.batch_numpy_to_vector(y_pred.cpu().numpy(), RASTER_RES)
    ...
    for epoch_i in range(epochs_completed, epochs_completed + options.epochs):
        for batch_i, batch_data in islice(enumerate(train_loader), batches_completed_in_epoch, len(train_loader)):
            ...
            if batch_i > 0 and batch_i % options.batches_before_val == 0:
                ...
                y_true_vector = vmetrics.batch_numpy_to_vector(y_true.cpu().numpy(), RASTER_RES)
                y_pred_vector = vmetrics.batch_numpy_to_vector(y_pred.detach().cpu().numpy(), RASTER_RES)
                ...
                logger.info_scalars('Computed {key} over last batch of {num_items} images: {value:.4f}', train_metrics, num_items=len(y_true))
