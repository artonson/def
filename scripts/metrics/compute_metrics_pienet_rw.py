...
def main(options):
    labels_gt = sio.loadmat(str(options.true_filename))['Training_data'][0,0]['PC_8096_edge_points_label_bin'][0,0]
    ...
    true_distances = [labels_gt]
    pred_distances = [pred_labels]
    ...
    values = []
    thresh_1r = options.resolution_3d
        ...
        for metric in metrics:
            if isinstance(metric, MFPR) or isinstance(metric, MRecall):
                metric.update(
                    ...
                    torch.tensor(true_item).reshape(1, -1) < thresh_1r
                )
            else:
                metric.update(
                    torch.tensor(pred_item).reshape(1, -1),
...
if __name__ == '__main__':
    ...
    options = parse_args()
    ARGS_PATH = "/home/appuser/ssd/ebogomolov/images_align4mm_pc/args_metrics.txt"
