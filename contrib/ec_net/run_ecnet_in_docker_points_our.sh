...
SPLIT_DATA_PATH_CONTAINER="${OUT_PATH_CONTAINER}/xyz_splitted"
SPLIT_INPUT_CONTAINER="${SPLIT_DATA_PATH_CONTAINER}/*.xyz"
SPLIT_OUTPUT_CONTAINER="${OUT_PATH_CONTAINER}/our_ec_net_results"
CODE_PATH_CONTAINER="/home/EC-Net/code"
MODEL_PATH_HOST="/trinity/home/a.matveev/EC-Net/model/myownmodel"
MODEL_PATH_CONTAINER="/home/EC-Net/model/new_model"

echo "******* LAUNCHING IMAGE ${IMAGE_NAME} IN CONTAINER ${CONTAINER_NAME} *******"
echo "  "
echo "  HOST OPTIONS:"
echo "  input path:           ${INPUT_FILE}"
echo "  output path:          ${OUT_PATH_HOST}/our_ec_net_results"
echo "  split code path:      ${SPLITCODE_PATH_HOST}"
echo "  logs path:            ${LOGS_PATH_HOST}"
echo "  model path:           ${MODEL_PATH_HOST}"
echo "  "
...
singularity exec \
    ...
    --bind "${MODEL_PATH_HOST}":"${MODEL_PATH_CONTAINER}" \
    "${IMAGE_NAME}" \
    /bin/bash -c "cd ${SPLITCODE_PATH_CONTAINER} && \\
        echo 'Splitting input files...' && \\
        python split_hdf5_points.py \\
       ...
       ls ${MODEL_PATH_CONTAINER} && \\
       echo ${MODEL_PATH_CONTAINER} && \\
       echo 'Output is in ${DATA_PATH_HOST}/our_ec_net_results'"
