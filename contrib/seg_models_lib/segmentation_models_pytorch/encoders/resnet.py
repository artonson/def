import torch
from .pd_resnet import PDResNet as ResNet#torchvision.models.resnet import ResNet
...
class ResNetEncoder(ResNet):
    ...
    def forward(self, x):
        mask = torch.reshape(x[:,-1, :, :], (x.shape[0],1, x.shape[-2], x.shape[-1]))
        #mask = torch.cat([mask, mask, mask], dim=-3)
        x = x[:, :-1, :, :]
        x0 = self.conv1(x, mask_in=mask)
        x0 = self.bn1(x0)
        x0 = self.relu(x0)
