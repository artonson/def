...
#import vectran.util.color_utils as color_utils
...
def _prepare_raster(raster, binarization=None, envelope=0):
   ...
   raster_arr = raster.cpu().detach().numpy()
   raster_arr = ensure_tensor(raster_arr)
   ...
   raster_arr = binarization_func(raster_arr)
   assert _is_binary_1bit(raster_arr), 'images are not binary (only values 0 and 1 allowed in images)'

   # invert images so that we compute a correct metric
   raster_arr = 1 - raster_arr

    # compute envelopes around ground-truth
    # to compute metrics with spatial tolerance
   if envelope:
       assert isinstance(envelope, int)
       raster_arr = np.array([binary_dilation(r, iterations=envelope)
                          for r in raster_arr]).astype(np.uint8)
   return raster_arr
