...
//
// $URL: https://github.com/CGAL/cgal/blob/releases/CGAL-4.14.1/Point_set_processing_3/include/CGAL/edge_aware_upsample_point_set.h $
// $Id: edge_aware_upsample_point_set.h 2f81a21 %aI Sébastien Loriot
  ...
  const NamedParameters& np)
{
  ...
  std::cout << rich_point_set[0].neighbors.size() << "\n";
  //
  ...
  std::ofstream points_file("points_2.txt");
  std::ofstream normals_file("normals_2.txt");
  std::ofstream sharp_classif("sharp_classif.txt");
  std::ofstream neighbors("neighbors.txt");
  std::vector<int> sharp_vector{};
  
  auto squared_dist = [](const std::pair<Rich_point, Rich_point>& i, const std::pair<Rich_point, Rich_point>& j){return (CGAL::squared_distance(i.first.pt, i.second.pt) < CGAL::squared_distance(j.first.pt, j.second.pt));};
  ...
  for (std::size_t i = number_of_input; i < rich_point_set.size(); ++i)
  {
    ...
    float count_normals = 0;
    float count_normals_2 = 0;
    std::vector<std::pair<Rich_point, Rich_point>> neighbor_vector{};
    for (unsigned int j = 0; j < v.neighbors.size(); ++j){
	neighbor_vector.push_back(std::make_pair(rich_point_set[v.neighbors[j]], v));
    }
    std::sort(neighbor_vector.begin(), neighbor_vector.end(), squared_dist);
    int num_neighbors = 0;      
    float weight;
    float dist_thresh = 0.005;

    for (unsigned int j = 0; j < v.neighbors.size(); ++j){
	Rich_point& neighbor = rich_point_set[v.neighbors[j]];
	double dist2 = CGAL::squared_distance(neighbor.pt, v.pt);
	if (dist2 > dist_thresh)
		continue;
	num_neighbors += 1;
	double scalar_product = 0;
	double scalar_prod_1 = 0;
	double scalar_prod_2 = 0;
        for (unsigned int k = 0; k < 3; ++k){
            scalar_prod_1 += neighbor.normal[k]*neighbor.normal[k];
	    scalar_prod_2 += normal[k]*normal[k];
            scalar_product += neighbor.normal[k]*normal[k];
        }
	float score = abs(1 - (scalar_product*scalar_product) / (scalar_prod_1*scalar_prod_2));
	//std::cout << (1 - (scalar_product*scalar_product) / (scalar_prod_1*scalar_prod_2)) << " " << CGAL::squared_distance(neighbor.pt, v.pt) << "\n";
//	std::cout << score << " " << dist2 << "\n";
	if ((score > 0.4)){
	    //std::cout << CGAL::squared_distance(neighbor.pt, v.pt) << "\n";
            count_normals += 1*(dist_thresh/dist2)*(dist_thresh/dist2);
	    //std::cout << (dist2/dist_thresh) << "\n";
	}

	if ((score < 0.1) && (dist2 < 0.001)){
            count_normals_2 += 1;
	}
    }
    ...
    if ((count_normals >= num_neighbors/4) && (count_normals_2 < 4)){//((count_normals / count_normals_2) >= 4){
//	std::cout <<  count_normals_2 << "\n";
	sharp_classif << "1\n";}
    else{
	sharp_classif << "0\n";}
    points_file << point << "\n";
